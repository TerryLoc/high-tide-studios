...
import {
  buildBookingClientContent,
  buildBookingInternalContent,
...
export default function Booking() {
  ...
  // Single source of truth for the price breakdown — previously the balance
  // calculation was duplicated (once for the on-screen deposit box, once again
  // inside the EmailJS send call), which meant the two could silently drift
  ...
  const validateForm = useCallback(() => {
    ...
    if (selectedDates.length === 0)
      newErrors.dates = 'Please select at least one preferred date';
    if (!formData.agreeDeposit)
      newErrors.agreeDeposit = 'You must agree to the deposit terms';
    setErrors(newErrors);
  ...
  const handleSubmit = useCallback(
    async (e) => {
      ...
      try {
        ...
        const packageTitle = `${selectedPackage.title} — ${selectedPackage.subtitle}`;
        const packagePrice = selectedPackage.price;
        const depositAmount = `€${priceBreakdown.deposit}`;
        const balanceDue = `€${priceBreakdown.balance}`;

        // Internal notification is the business-critical email — the studio
        // needs to know about every booking request regardless of whether the
        // client's confirmation copy succeeds. Send both concurrently rather
        // than sequentially (previously one blocking call), and only fail the
        // whole submission if the internal one fails.
        const internalSend = emailjs.send(
          ...
          EMAILJS_INTERNAL_TEMPLATE_ID,
          {
            ...
            eyebrow_text: 'New Booking Request',
            content_html: buildBookingInternalContent({
        ...
        const clientSend = emailjs.send(
          ...
          EMAILJS_CLIENT_TEMPLATE_ID,
          {
            ...
            eyebrow_text: 'Studio Booking',
            content_html: buildBookingClientContent({
              fromName: formData.name,
              packageTitle,
              preferredDates: formattedDates,
              packagePrice,
        ...
        if (clientResult.status === 'rejected') {
          // The studio still got the booking — don't block the success screen
          // over the client's confirmation copy failing, just log it so it's
          // visible in error tracking/console rather than silently lost.
          console.warn(
            'Booking client confirmation email failed to send:',
            clientResult.reason,
          );
        }
        ...
        setErrors({
          submit: `Failed to send booking request: ${error.text || error.message || 'Unknown error'}. Please contact us directly at ${siteConfig.contact.email}`,
        });
      } finally {
        setIsSubmitting(false);
      }
    },
    [formData, selectedDates, priceBreakdown, selectedPackage, validateForm],
  );
  ...
  const renderCalendar = () => {
    ...
    dayNames.forEach((day) =>
      days.push(
        <div key={`header-${day}`} className="ht-cal-header">
          {day}
        </div>,
      ),
    );
    ...
    for (let i = 0; i < startingDay; i++) {
      days.push(
        <div key={`empty-${i}`} className="ht-cal-day ht-cal-day--empty" />,
      );
    }
    ...
    for (let day = 1; day <= daysInMonth; day++) {
      ...
      days.push(
        <div
          ...
          onKeyDown={(e) =>
            e.key === 'Enter' && handleDateClick(dateKey, year, month, day)
          }>
          {day}
        </div>,
      );
                ...
                <FadeInUp>
                  <div className="ht-booking-success">
                    ...
                    <p className="ht-body-text mb-4">
                      We'll review your preferred dates and contact you within
                      24 hours to confirm availability and arrange your{' '}
              ...
              <FadeIn delay={0.1}>
                <div className="ht-booking-card">
                  ...
                  <p className="ht-muted-text small mb-4">
                    Choose up to 3 preferred dates. We'll confirm availability
                    within 24 hours.
                  </p>
                  ...
                  <div className="ht-cal-nav">
                    <button
                      className="ht-cal-nav-btn"
                      onClick={prevMonth}
                      aria-label="Previous month">
                      <i className="bi bi-chevron-left" />
                    </button>
                    <h3 className="ht-cal-month">
                  ...
                  {/* Calendar grid */}
                  <div className="ht-cal-grid">{renderCalendar()}</div>

                  {/* Legend */}
                  <div className="ht-cal-legend">
                    <div className="ht-legend-item">
                      <span className="ht-legend-dot ht-legend-dot--available" />
                      Available
                    </div>
                    <div className="ht-legend-item">
                      <span className="ht-legend-dot ht-legend-dot--selected" />
                      Selected
                    </div>
                    <div className="ht-legend-item">
                      <span className="ht-legend-dot ht-legend-dot--unavailable" />
                      Unavailable
                    </div>
                    <div className="ht-legend-item">
                      <span className="ht-legend-dot ht-legend-dot--weekend" />
                      Weekend (by request)
                    </div>
                  </div>
                  ...
                  {selectedDates.length > 0 && (
                    <div className="ht-selected-dates mt-4">
                      <p className="ht-selected-dates-label">
                ...
                <div className="ht-booking-card">
                  ...
                  <form onSubmit={handleSubmit} noValidate>
                    ...
                    <div className="ht-field mb-3">
                      <label htmlFor="booking-name" className="ht-label">
                      ...
                      />
                      {errors.name && (
                        <p className="ht-field-error">{errors.name}</p>
                      )}
                    </div>

                    <div className="ht-field mb-3">
                      <label htmlFor="booking-email" className="ht-label">
                      ...
                      />
                      {errors.email && (
                        <p className="ht-field-error">{errors.email}</p>
                      )}
                    </div>

                    <div className="ht-field mb-3">
                      <label htmlFor="booking-phone" className="ht-label">
                      ...
                      <input
                        ...
                        onChange={handleChange}
                        className={`ht-input ${errors.phone ? 'ht-input--error' : ''}`}
                        placeholder="087 123 4567"
                        autoComplete="tel"
                      />
                      {errors.phone && (
                        <p className="ht-field-error">{errors.phone}</p>
                      )}
                    </div>

                    <div className="ht-field mb-3">
                      <label htmlFor="booking-company" className="ht-label">
                    ...
                    <div className="ht-field mb-3">
                      <label htmlFor="booking-package" className="ht-label">
                        Select Package <span aria-hidden="true">*</span>
                      </label>
                      <select
                        id="booking-package"
                        name="package"
                      ...
                      {errors.package && (
                        <p className="ht-field-error">{errors.package}</p>
                      )}
                    </div>

                    <div className="ht-field mb-4">
                      <label htmlFor="booking-notes" className="ht-label">
                        Additional Notes{' '}
                        <span className="ht-optional">(optional)</span>
                      </label>
                      <textarea
                    ...
                    {selectedPackage && priceBreakdown && (
                      <div className="ht-deposit-box mb-4">
                        ...
                        <div className="ht-deposit-row">
                          <span>Deposit (10%)</span>
                          <span className="ht-deposit-amount">
                            €{priceBreakdown.deposit}
                          </span>
                        </div>
                        ...
                        <div className="ht-deposit-row">
                          ...
                          <span>€{priceBreakdown.balance}</span>
                        </div>
                      </div>
                    )}
                    ...
                    <div className="ht-field mb-4">
                      <label className="ht-checkbox-label">
                        <input
                          ...
                          onChange={handleChange}
                          className={`ht-checkbox ${errors.agreeDeposit ? 'ht-checkbox--error' : ''}`}
                          id="booking-agree"
                        />
                        <span>
                          I understand that a{' '}
                          <strong>10% non-refundable deposit</strong> is
                          required to secure my booking. The remaining balance
                          is due 48 hours before the day of the first recording.
                        </span>
                      </label>
                      {errors.agreeDeposit && (
                        <p className="ht-field-error">{errors.agreeDeposit}</p>
                      )}
                    </div>
